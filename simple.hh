...
#endif
#if 201103L <= YY_CPLUSPLUS
      basic_symbol (typename Base::kind_type t, symbol_t*&& v, location_type&& l)
        : Base (t)
        , value (std::move (v))
        , location (std::move (l))
      {}
...
#endif
#if 201103L <= YY_CPLUSPLUS
      basic_symbol (typename Base::kind_type t, vector<symbol_t*> && v, location_type&& l)
        : Base (t)
        , value (std::move (v))
        , location (std::move (l))
      {}
...
switch (yytype)
    ...
    struct symbol_type : basic_symbol<by_type>
    {
      ...
      {
        YYASSERT (tok == token::T_EOF || tok == token::T_NUM || tok == token::T_ASSIGN || tok == token::T_ADD || tok == token::T_SUB || tok == token::T_MUL || tok == token::T_DIV || tok == token::T_DT_INT || tok == token::T_DT_FLOAT || tok == token::T_LITERAL_STR || tok == token::T_READ || tok == token::T_WRITE || tok == token::T_SEMICOLON || tok == token::T_COMMA || tok == 40 || tok == 41);
      }
#else
    ...
    // Implementation of make_symbol for each symbol type.
      ...
      static
      symbol_type
      make_T_DT_INT (location_type l)
      {
        return symbol_type (token::T_DT_INT, std::move (l));
      }
#else
      static
      symbol_type
      ...
      static
      symbol_type
      make_T_DT_FLOAT (location_type l)
      {
        return symbol_type (token::T_DT_FLOAT, std::move (l));
      }
#else
      static
      symbol_type
      ...
      static
      symbol_type
      make_T_LITERAL_STR (location_type l)
      {
        return symbol_type (token::T_LITERAL_STR, std::move (l));
      }
#else
      static
      symbol_type
      ...
      static
      symbol_type
      make_T_INTEGER (int v, location_type l)
      {
      ...
      static
      symbol_type
      make_T_FLOAT (float v, location_type l)
      {
        return symbol_type (token::T_FLOAT, std::move (v), std::move (l));
      }
#else
      static
      symbol_type
...
#if 201103L <= YY_CPLUSPLUS
  template <typename Base>
  simple_parser::basic_symbol<Base>::basic_symbol (basic_symbol&& that)
    : Base (std::move (that))
    , value ()
  ...
  template <typename Base>
  simple_parser::basic_symbol<Base>::basic_symbol (const basic_symbol& that)
    : Base (that)
    , value ()
  ...
  template <typename Base>
  bool
  simple_parser::basic_symbol<Base>::empty () const YY_NOEXCEPT
  {
...
#if 201103L <= YY_CPLUSPLUS
  inline
  simple_parser::by_type::by_type (by_type&& that)
    : type (that.type)
  {
    that.clear ();
  }
  ...
  inline
  simple_parser::by_type::by_type (const by_type& that)
    : type (that.type)
  {}
  ...
  inline
  int
  simple_parser::by_type::type_get () const YY_NOEXCEPT
  {
    return type;
  }
  ...
  simple_parser::by_type::token () const YY_NOEXCEPT
  {
